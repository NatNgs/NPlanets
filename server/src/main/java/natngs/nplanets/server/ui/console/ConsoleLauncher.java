...
import natngs.nplanets.server.located.planets.RoundOrbit2DPlanet;
...
public class ConsoleLauncher {
	...
	private void demo(int turns) {
		...
		gc.put("Sun", new StaticPlanet(u, new Location(0, 0)));

		// Orbit duration in months (12*earth orbit duration)
		gc.put("Mercury", new RoundOrbit2DPlanet(u, new Location(0, 0.387), 12 * 0.241));
		gc.put("Venus", new RoundOrbit2DPlanet(u, new Location(0, 0.723), 12 * 0.615));
		gc.put("Earth", new RoundOrbit2DPlanet(u, new Location(0, 1.000), 12 * 1.000));
		gc.put("Mars", new RoundOrbit2DPlanet(u, new Location(0, 1.523), 12 * 1.881));
		gc.put("Jupiter", new RoundOrbit2DPlanet(u, new Location(0, 5.204), 12 * 11.862));
		gc.put("Saturn", new RoundOrbit2DPlanet(u, new Location(0, 9.583), 12 * 29.457));
		gc.put("Uranus", new RoundOrbit2DPlanet(u, new Location(0, 19.218), 12 * 84.021));
		gc.put("Neptune", new RoundOrbit2DPlanet(u, new Location(0, 30.110), 12 * 164.8));
		gc.put("Pluto", new RoundOrbit2DPlanet(u, new Location(0, 39.48), 12 * 248.000));
		...
		try {
			...
			Ship voyager = new Ship(steps, 3.6 * 2 / 12, 0);
